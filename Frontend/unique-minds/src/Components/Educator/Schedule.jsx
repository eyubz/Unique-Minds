...
const Schedule = () => {
    ...
    const fetchSchedules = async () => {
      ...
      try {
        const response = await fetch(
          "https://unique-minds.onrender.com/api/educator/schedules",
          {
            headers: {
  ...
  const handleCancelSchedule = async (id) => {
    try {
      const token = localStorage.getItem("access_token");
      const response = await fetch(
        `https://unique-minds.onrender.com/api/educators/schedules/${id}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {schedules !== null && schedules.length > 0 ? (
          schedules.map((schedule) => (
            <div
              key={schedule.id}
              className="flex flex-col justify-between bg-gray-50 p-6 rounded-lg shadow hover:shadow-md transition-shadow duration-300"
            >
